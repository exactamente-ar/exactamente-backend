...
import { PaginatedSubjectSchema, SubjectDetailResponseSchema } from '@/schemas';
import { json, errors } from '@/openapi/helpers';
...
app.get(
  ...
  describeRoute({
    tags: ['Materias'],
    summary: 'Listar materias con sus carreras',
    description:
      'Ordenadas por año, cuatrimestre y título. `search` es case-insensitive y sin acentos.',
    responses: {
      200: json(PaginatedSubjectSchema, 'Materias con carreras y contadores de recursos'),
    },
  }),
  publicReadLimit,
  zValidator('query', subjectFiltersSchema),
  async (c) => {
...
app.get(
  ...
  describeRoute({
    ...
    responses: {
      200: json(SubjectDetailResponseSchema, 'Materia con sus carreras'),
      ...errors(404),
    },
  }),
  publicReadLimit,
    ...
    return c.json({
      subject: {
        ...
        careers: subject.careerSubjects.map((cs) => ({
          ...
          universityId: cs.career.faculty.universityId,
          universityName:
            cs.career.faculty.university.shortName ?? cs.career.faculty.university.name,
          planId: cs.planId,
          year: cs.year,
          quadmester: cs.quadmester,
        })),
