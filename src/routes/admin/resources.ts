...
app.get(
  ...
  describeRoute({
    tags: ['Admin'],
    summary: 'Listar recursos en cualquier estado',
    security: bearerAuth,
    responses: {
      200: json(
        PaginatedAdminResourceSchema,
        'Recursos, filtrables por status, subjectId y careerId',
      ),
      ...errors(401, 403),
    },
  }),
  ...
  async (c) => {
    ...
    if (careerId) {
      const sub = db
        .select({ id: careerSubjects.subjectId })
        .from(careerSubjects)
    ...
    const [rows, countResult] = await Promise.all([
      db
        .select({ resource: resources, subjectTitle: subjects.title })
        .from(resources)
    ...
    const subjectToCareer = new Map<
      string,
      { careerName: string; planName: string; planYear: number }
    >();
    for (const cs of csRows) {
      if (!subjectToCareer.has(cs.subjectId)) {
        subjectToCareer.set(cs.subjectId, {
          careerName: cs.careerName,
          planName: cs.planName,
          planYear: cs.planYear,
        });
      }
    }
    ...
    return c.json(
      buildPaginatedResponse(
        rows.map((r) =>
          rowToAdminResource(
            r.resource,
            r.subjectTitle ?? undefined,
            subjectToCareer.get(r.resource.subjectId),
          ),
        ),
        total,
        safePage,
...
app.post(
  ...
  describeRoute({
    tags: ['Admin'],
    summary: 'Subir recurso como admin',
    security: bearerAuth,
    description: 'Publica directo salvo que se mande `autoPublish=false`, que lo deja pending.',
    responses: { 201: json(AdminResourceSchema, 'Recurso creado'), ...errors(400, 401, 403, 404) },
  }),
  ...adminGuard,
  async (c) => {
...
app.patch(
  ...
  describeRoute({
    tags: ['Admin'],
    summary: 'Aprobar recursos en lote',
    security: bearerAuth,
    description:
      'Operación parcial: procesa los 1-100 ids y los errores individuales no detienen al resto.',
    responses: {
      200: json(BulkApproveResponseSchema, 'Aprobados y errores por id'),
      ...errors(400, 401, 403),
    },
  }),
  ...adminGuard,
...
app.get(
  ...
  describeRoute({
    ...
    security: bearerAuth,
    responses: { 200: json(AdminResourceSchema, 'Recurso'), ...errors(401, 403, 404) },
  }),
  ...adminGuard,
  async (c) => {
    ...
    const subject = await db.query.subjects.findFirst({
      where: eq(subjects.id, resource.subjectId),
    });
    ...
    return c.json({
      ...rowToAdminResource(resource, subject?.title ?? undefined),
      similarResources,
    });
  },
);
...
app.get(
  ...
  describeRoute({
    tags: ['Admin'],
    summary: 'URL firmada para previsualizar el PDF',
    security: bearerAuth,
    description: 'La URL es temporal. Permite revisar el archivo antes de aprobarlo.',
    responses: { 200: json(SignedUrlResponseSchema, 'URL temporal'), ...errors(401, 403, 404) },
  }),
  ...adminGuard,
  async (c) => {
    const id = c.req.param('id');
...
app.delete(
  ...
  describeRoute({
    ...
    security: bearerAuth,
    responses: { 204: { description: 'Eliminado' }, ...errors(401, 403, 404) },
  }),
  ...adminGuard,
  async (c) => {
    const id = c.req.param('id');
...
app.patch(
  ...
  describeRoute({
    tags: ['Admin'],
    summary: 'Editar metadatos de un recurso',
    security: bearerAuth,
    responses: {
      200: json(AdminResourceSchema, 'Recurso actualizado'),
...
function rowToAdminResource(
  ...
  return {
    ...
    fileUrl: row.r2Key && row.status === 'published' ? storage.getPublicUrl(row.r2Key) : null,
  };
}
